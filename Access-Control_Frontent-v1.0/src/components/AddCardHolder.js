...
const AddCardHolder = () => {
  ...
  const uploadFromXLSX = (e) => {
    ...
    reader.onload = (evt) => {
      ...
      console.log(data);
      if (
        data["0"].every((x) => xlsxHeader.includes(x)) &&
        data["0"].length === 8
      ) {
        setXlsxHolders(data);
      } else {
  ...
  const registerCardHolder = async () => {
    await axios
      .post(`${url}:5000/registerch`, { ...cardHolder, selectedTimezone })
    ...
    <Container className="mt-2">
      ...
      >
        ...
        <Form.Control
          ...
          className="d-none"
          id="upxlsx"
          onChange={uploadFromXLSX}
        />
        <Form.Label htmlFor="upxlsx" className="btn btn-success m-0 ms-auto">
          დაამატეთ Excel-დან
        </Form.Label>
      </Col>
      <Form
      ...
      >
          ...
          <Form.Group className="col">
            ...
            <InputGroup>
              ...
              <Form.Control
                ...
                value={cardHolder.PrivateNumber}
            ...
            <Col className="mt-2 d-flex gap-4">
              <Col className="d-flex col-auto gap-1">
                <Form.Check
                  ...
                  checked={cardHolder.Gender === "true"}
                />
              ...
              <Col className="d-flex col-auto gap-1">
                <Form.Check
                  ...
                  value={false}
                  onChange={(e) => editPersonalInfo(e, "Gender")}
                  checked={cardHolder.Gender === "false"}
                />
            ...
            <InputGroup>
              ...
              <Col className="d-flex align-items-center justify-content-center gap-1 ms-3">
                <Form.Check
                  id="disableUID"
                  type="checkbox"
                  onChange={(e) => editPersonalInfo(e, "Activated")}
                  checked={cardHolder.Activated}
                />
                <Form.Label
                  htmlFor="disableUID"
        ...
        <Col className="d-flex gap-3 mb-4">
          <Button variant="dark" onClick={() => setJoinTimezoneModal(true)}>
            აირჩიეთ სამუშაო დროები
          </Button>
          <Button variant="dark" onClick={() => setTimezoneModal(true)}>
            ახალი დროის დამატება
          </Button>
        </Col>
        <Col>
          <Button
      ...
      <XLSXTable
        xlsxModal={xlsxModal}
        hideXlsxModal={hideXlsxModal}
        xlsxHolders={xlsxHolders}
        url={url}
      />
      <XLSXMessage
        xlsxMessage={xlsxMessage}
        hideXlsxMessage={hideXlsxMessage}
      />
      {joinTimezoneModal && (
        <JoinTimeZone
          joinTimezoneModal={joinTimezoneModal}
          finishJoinTimezone={hideJoinTimezone}
          selectedTimezone={selectedTimezone}
          setSelectedTimezone={setSelectedTimezone}
          finishButtonName={"დასრულება"}
        />
      )}
      {timezoneModal && (
        <AddTimeZoneGraph
...
const XLSXTable = ({ xlsxModal, hideXlsxModal, xlsxHolders, url }) => {
  ...
  const registerFromXLSX = async () => {
    setIsLoading(true);
    await axios
      .post(`${url}:5000/registerfromxlsx`, xlsxHolders)
      .then((res) => {
        if (res.status === 200) {
          res.data.length === 0
            ? setExistingHolders([10n])
            : setExistingHolders(res.data);
          setIsLoading(false);
        }
      })
      .catch(console.error);
                  ...
                  if (i > 0) {
                    ...
                    return (
                      <tr className={`${updateColor}`}>
                        {x.map((y, j) => {
                          return j === 6 ? (
                            <td>{new Date(y).toLocaleDateString("en-GB")}</td>
                          ) : (
                            <td>{y}</td>
                          );
                        })}
      ...
      <Modal.Footer>
        ...
        {!isLoading && (
          <>
            ...
            <Button variant="success" onClick={registerFromXLSX}>
              შენახვა
            </Button>
          </>
        )}
      ...
      <Modal.Body>
        <Col className="d-flex flex-column gap-3">
          <Col className="text-center">
            Excel-ის სათაურები უნდა შედგებოდეს ფოტოში მოცემული მიმდევრობით და
            სახელწოდებით
          </Col>
          <Col>
          ...
          <Col>
            <b>Headers:</b> FirstName, LastName, IdentNumber, PersonalNumber,
